...
@ALL_MAPS
def test_compute_weights(datafiles):
    """Test compute weights for different issues:
        1) normal
        2) with rank deficiency
        3) one all zero column
    """
    n_sample = random.randint(4, 5)
    landcover_map = get_file(pattern="Switzerland_CLC_*.tif", datafiles=datafiles)
    ndvi_map = get_file(pattern="Switzerland_NDVI_*.tif", datafiles=datafiles)
    lbio.coregister_raster(ndvi_map, landcover_map, output=str(ndvi_map)) # rescale to 100m
    lct_source = lbio_.Source(path=landcover_map)
    categories = random.sample([1, 2, 3, 4, 5, 6, 7, 8], n_sample)
    diameter = 1000  # 1km
    blur_params = dict(
        sigma=(0.5 * diameter / 3) / 100,  # 100=scale in pixel
        truncate=3)
    ...
    blur_source = lbio_.Source(path=blurred_tif)
    predictors = [blur_source.get_band(category=c) for c in categories]
    lbpara.compute_mask(blur_source,
                        block_size=(500, 500))
    for p in predictors:
        p.set_mask_reader(use="source")

    # 1) Normal test
    result = lbpara.compute_weights(response=ndvi_map,
                                    predictors=predictors,
                                    block_size=(500, 500),
                                    include_intercept=False)
    assert set(predictors) == set([k for k in result.keys()])

    # 2) Linear dependency
    # Modify one band (to be linear dependent of other)
    n_dep = random.randint(3, n_sample)
    pred_sample_dep = random.sample(predictors, n_dep)
    ref_array = pred_sample_dep[0].get_data()
    with rio.open(blurred_tif, mode='r+') as dst:
        for i in range(1, n_dep):
    ...
    assert set([k for k in result['linear_dependent'].keys()]) == set(pred_sample_dep)

    # 3) All zero band
    # This should get cought by sanitize predictors
    pred_sample_zero = random.sample(pred_sample_dep, 1)[0]
    ref_array = pred_sample_zero.get_data()
    zero_array = np.zeros(ref_array.shape)
    with rio.open(blurred_tif, mode='r+') as dst:
        dst.write(zero_array, indexes=pred_sample_zero.get_bidx())
