...
def test_optimal_weights_test_data():
    ...
    b_noisy = np.round(lfinf.get_optimal_weights(X_noisy, y), 1)
...
def test_weights_computations_test_data():
    ...
    reg = lfinf.get_approx_weights(X, y)
    ...
    # this should affect the quality of the fit (so we round to first digit:
    b_noisy = np.round(lfinf.get_optimal_weights(X_noisy, y), 1)
    reg_noisy = lfinf.get_approx_weights(X_noisy, y)
...
def test_transposed_prod_example_data(datafiles, create_blurred_tif,
    ...
    selector = lfinf.prepare_selector(response,
                                      *predictors)
    transprodX = lfinf.transposed_product(predictors,
                                          view=None,
                                          selector=selector,
                                          as_dtype="float64")
...
def test_transposed_prod_blurred_example_data(datafiles, create_blurred_tif):
    ...
    selector = lfinf.prepare_selector(response,
                                      *predictors,
                                      verbose=verbose)
    riow = rghelp.view_to_window(view)
    X = lfinf.init_X(predictors,
                     selector=selector,
                     window=riow,
...
def test_optimal_beta(datafiles, create_blurred_tif):
    ...
    for isetup in intercept_switch:
        ...
        selector = lfinf.prepare_selector(response,
                                          *predictors)
        tpX_col = lfinf.transposed_product(predictors,
                                           view=None,
                                           include_intercept=isetup,
                                           selector=selector,
                                           as_dtype=as_dtype)
        Y_col = np.linalg.inv(tpX_col)
        betas_col = lfinf.get_optimal_weights_source(Y=Y,
                                                     response=response,
                                                     predictors=predictors,
                                                     view=None,
                                                     include_intercept=isetup,
