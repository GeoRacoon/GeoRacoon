...
separate categories, apply a gaussian filter on each band and compute the
...
entropy_data = entropy(combined, axis=2)

###
# Faster
###
# We can do the same thing with a single command:
entropy_data_fast = lbproc.get_entropy(
    rand_data,
    categories=[2, 3],
    normed=False,
    img_filter=gaussian,
...
print('Differences:')
print(np.unique(entropy_data_fast - entropy_data))
